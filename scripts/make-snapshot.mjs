...
async function main() {
  ...
  const manifest = {
    maker: rounds.maker,
    book: rounds.book,
    rounds: rounds.rounds.map((r) => ({ round: r.round, orderHash: r.orderHash })),
  };
  writeFileSync(
    new URL("../site/data/rounds.js", import.meta.url),
    "// GENERATED by scripts/make-snapshot.mjs from config/rounds.json. Do not edit.\n" +
      "// Order hashes only: the page reads each round's live state from the chain.\n" +
      `window.GLASSHOUSE_ROUNDS = ${JSON.stringify(manifest)};\n`,
    "utf8",
  );
  console.log(`  wrote site/data/rounds.js with ${manifest.rounds.length} round hashes\n`);
}
