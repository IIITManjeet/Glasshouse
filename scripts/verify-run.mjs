...
// --emit writes the result where the SITE can read it, which is the whole point of having
// it. A judge will not open a terminal, and this script is the strongest evidence the
// project has; leaving it behind a CLI meant the evidence existed and nobody saw it.
...
async function main() {
  ...
  // The same artifact pattern as scripts/make-snapshot.mjs: a generated file in site/data/
  // assigning a global, copied into the app by web/scripts/sync-assets.mjs. It is a
  // SNAPSHOT OF A RUN, never a live claim -- it carries the block it was taken at and the
  // RPC it asked, and the panel that renders it says both.
  if (EMIT) {
    const emitPath = new URL("../site/data/verification.js", import.meta.url);
    const payload = {
      ...
      notApplicable: results.filter((r) => r.ok === null).length,
      checks: results,
    };
    const header = [
    ...
    writeFileSync(
      emitPath,
      header + `window.GLASSHOUSE_VERIFICATION = ${JSON.stringify(payload, null, 2)};\n`,
      "utf8",
    );
    console.log("  emitted site/data/verification.js (block " + head + ")\n");
  }
