...
### 2.3 External function reference

Throughout, `a` is `_auctions[key(maker, orderHash)]` and `b` is
`_bids[key(maker, orderHash)][msg.sender]`.

---

#### `open(bytes32 orderHash, address router, address tokenIn, uint40 commitBlocks, uint40 revealBlocks, uint40 exclusiveBlocks, uint24 reserveBps, uint24 maxBps, uint128 bond)`

`src/book/GlasshouseBook.sol:108-142`. Caller is the maker.

Preconditions, in evaluation order:

| Check | Line | Error |
|---|---|---|
| `a.commitEnd == 0` (never opened) | 121 | `AlreadyOpened()` |
| `commitBlocks > 0 && revealBlocks > 0 && exclusiveBlocks > 0` | 126 | `BadWindow()` |
| `reserveBps <= maxBps && maxBps < 10_000` | 127 | `BadWindow()` |

State transition: writes `router`, `tokenIn`, `commitEnd = uint40(block.number) + commitBlocks`,
`revealEnd = commitEnd + revealBlocks`, `exclusiveBlocks`, `reserveBps`, `maxBps`, `bond`
(lines 129-139). Nothing else in the struct is touched, so `best`, `second`,
`commitCount`, `filledBy`, `settled` and `winnerForfeited` are all zero.

Event: `AuctionOpened` (line 141).

Notes:

- `commitEnd` doubles as the existence sentinel. Because `commitBlocks > 0` is enforced
  and `block.number` is at least 1 on any live chain, `commitEnd` is always nonzero for
  an opened auction, so the sentinel cannot be spoofed by a legitimate configuration.
- `exclusiveBlocks > 0` is load-bearing, not cosmetic. With a zero-length window an
  outsider fills at the base price in the same block the winner would fill at the
  improved price, so bidding is strictly dominated by not bidding and the auction has no
  bidders (`src/book/GlasshouseBook.sol:122-125`, `run.md` F-120 and F-134). Pinned by
  `test/GlasshouseBook.t.sol:129-133`.
- `maxBps < 10_000` bounds the improvement below 100 percent, so the `(BPS + clearingBps)`
  factor in §4 stays under 2x.
- The two `uint40` additions on lines 129-130 are checked arithmetic. A maker passing a
  window near `2**40` gets a `Panic(0x11)` rather than `BadWindow()`. See §12, G-6.
- There is no zero-address check on `router` or `tokenIn`. See §12, G-3, which is the
  most consequential gap found.

---

#### `commit(address maker, bytes32 orderHash, bytes32 commitment)`

`src/book/GlasshouseBook.sol:148-162`. Caller is the bidder.

| Check | Line | Error |
|---|---|---|
| `a.commitEnd != 0` | 151 | `NotOpened()` |
| `block.number <= a.commitEnd` | 152 | `CommitClosed()` |
| `b.commitment == bytes32(0)` | 155 | `AlreadyCommitted()` |

State transition: `b.commitment = commitment`, `b.commitIdx = a.commitCount`, then
`a.commitCount++` inside `unchecked` (lines 157-159). The increment is safe to leave
unchecked: overflowing a `uint40` counter requires 2**40 distinct committing addresses
against one auction.

Event: `BidCommitted` (line 161).

The contract never inspects `commitment`, so committing garbage is allowed; it simply
cannot be revealed. One commitment per bidder per auction, and no replacement: the
`AlreadyCommitted` check makes a commit final. That, together with the tie-break on
`commitIdx`, is what makes tie resolution a fact fixed before anyone could know they were
tying.

---

#### `reveal(address maker, bytes32 orderHash, uint24 bps, bytes32 salt)`

`src/book/GlasshouseBook.sol:169-206`. Caller is the bidder.

| Check | Line | Error |
|---|---|---|
| `a.commitEnd != 0` | 172 | `NotOpened()` |
| `block.number > a.commitEnd` | 173 | `RevealNotOpen()` |
| `block.number <= a.revealEnd` | 174 | `RevealClosed()` |
| `bps >= a.reserveBps && bps <= a.maxBps` | 175 | `BidOutOfRange(bps, reserveBps, maxBps)` |
| `b.commitment != bytes32(0)` | 178 | `NoCommitment()` |
| `!b.revealed` | 179 | `AlreadyRevealed()` |
| `b.commitment == keccak256(abi.encodePacked(msg.sender, bps, salt))` | 180 | `BadReveal()` |
| ERC-20 `transferFrom` of `bond`, if `bond > 0` | 185 | bubbles up from `SafeERC20` |

State transition: `b.revealed = true` (line 182), bond escrowed (lines 184-185), then the
top-2 update of §2.4 (lines 194-203).

Event: `BidRevealed(maker, orderHash, msg.sender, bps, bond)` (line 205).

The commitment binds `msg.sender`, so a commitment lifted from the mempool cannot be
replayed by a different address: the copier's `keccak256(abi.encodePacked(copier, bps, salt))`
will not match. Pinned by `test/GlasshouseBook.t.sol:226-238`.

The range check runs before the commitment check, which means the revert selector leaks
whether a bid is in range before proving knowledge of the preimage. That is not a leak of
anything private: `reserveBps` and `maxBps` are public and the caller supplies `bps`.

Note that the bond moves at reveal, not at commit. This is the mechanism gap G-4 in §12.

---

#### `outcome(address maker, bytes32 orderHash) returns (Outcome)`

`src/book/GlasshouseBook.sol:210-230`. `external view`. This is the only function the
instruction calls.

Four exit paths, in order:

| Condition | Line | Returned `Outcome` |
|---|---|---|
| `a.commitEnd == 0` | 213-215 | `(None, 0, 0, 0)` |
| `block.number <= a.revealEnd` | 216-218 | `(Bidding, 0, 0, 0)` |
| `a.best == address(0)` | 219-222 | `(Closed, 0, 0, a.revealEnd)` |
| otherwise | 227-229 | `(Closed, a.best, max(a.secondBps, a.reserveBps), a.revealEnd + a.exclusiveBlocks)` |

Cost: at most four cold `SLOAD`s (slots 1, 2, 3, 4 of the `Auction` struct) plus the
`keccak256` for the key. No loop, no unbounded work, no dependence on bidder count. This
is what the running top-2 buys: `outcome()` shares the taker's gas budget, so a scan over
bidders would be a griefing vector.

The `no winner` arm returns `exclusiveUntil = a.revealEnd`. That value can never gate
anything, because the instruction only ever runs when `block.number > revealEnd`, so the
`block.number <= o.exclusiveUntil` test in §4 is false by construction. Returning
`revealEnd` rather than 0 is therefore equivalent to returning 0 here, and it keeps the
field's meaning consistent ("the window ended at this block").

`Outcome` is a memory struct of four fields (`src/interfaces/IGlasshouseBook.sol:26-31`).
Since the call crosses an ABI boundary, it is returned as four 32-byte words; the
`uint24`/`uint40` widths save storage, not calldata.

---

#### `postTransferIn(address maker, address taker, address, address, uint256 amountIn, uint256 amountOut, uint256, bytes32 orderHash, bytes calldata, bytes calldata)`

`src/book/GlasshouseBook.sol:236-256`. Implements
`node_modules/@1inch/swap-vm/src/interfaces/IMakerHooks.sol:45`. Caller must be the
configured router.

| Check | Line | Error |
|---|---|---|
| `msg.sender == a.router` | 250 | `NotRouter()` |

State transition: if `a.filledBy == address(0)`, set it to `taker` and emit
`AuctionFilled` (lines 252-255). Subsequent calls are silent no-ops, so a partially
fillable order that is hit several times records only its first filler. Pinned by
`test/GlasshouseBook.t.sol:503-511`.

If no auction was ever opened, `a.router` is zero and the check reverts with `NotRouter`,
since `msg.sender` cannot be the zero address. So the hook is safe to wire on an order
with no auction, though there is no reason to.

The three unused `IMakerHooks` legs are empty bodies
(`src/book/GlasshouseBook.sol:323-325`); they exist to satisfy the interface. They are
unguarded and do nothing, which is correct but worth naming: anyone may call them, and
nothing happens.

---

#### `settle(address maker, bytes32 orderHash)`

`src/book/GlasshouseBook.sol:261-274`. Permissionless.

| Check | Line | Error |
|---|---|---|
| `a.commitEnd != 0` | 263 | `NotOpened()` |
| `!a.settled` | 264 | `AlreadySettled()` |
| `block.number > a.revealEnd + a.exclusiveBlocks` | 265 | `WindowNotElapsed()` |

State transition: `a.settled = true` (267), then
`a.winnerForfeited = (a.best != address(0) && a.filledBy != a.best)` (270).

Event: `AuctionSettled(maker, orderHash, a.best, a.best == 0 ? 0 : clearingBps, a.winnerForfeited)`
(273), recomputing `clearingBps` from frozen storage on line 272 with the same expression
`outcome()` uses.

The forfeiture predicate is deliberately blunt: the winner forfeits unless the recorded
filler is exactly the winner. If an outsider somehow filled during the window, or if
nobody filled at all, the winner forfeits either way
(`test/GlasshouseBook.t.sol:583-593`). The instruction is supposed to make the outsider
case impossible; the bond answers for it if it is not.

`settle()` becomes callable in exactly the first block in which the order is also open to
every taker, so the two events (exclusivity ending, settlement becoming possible)
coincide by construction rather than by coordination.

---

#### `claimBond(address maker, bytes32 orderHash)`

`src/book/GlasshouseBook.sol:277-291`. Caller is the bidder.

| Check | Line | Error |
|---|---|---|
| `a.settled` | 280 | `NotSettled()` |
| `b.revealed && !b.bondClaimed` | 283 | `NothingToClaim()` |
| `!(a.winnerForfeited && msg.sender == a.best)` | 284 | `NothingToClaim()` |

State transition: `b.bondClaimed = true` (286), then `safeTransfer` of `a.bond` if
nonzero (288). Checks-effects-interactions is respected: the flag is set before the
external call, so a reentrant token cannot double-claim.

Event: `BondClaimed` (290).

A bidder who committed but never revealed has `b.revealed == false` and is refused. They
posted nothing, so there is nothing to refund.

---

#### `claimForfeit(bytes32 orderHash)`

`src/book/GlasshouseBook.sol:298-312`. Caller is the maker; the key is derived from
`msg.sender`, so a maker can only ever collect on its own auction.

| Check | Line | Error |
|---|---|---|
| `a.settled` | 301 | `NotSettled()` |
| `a.winnerForfeited` | 302 | `NothingToClaim()` |
| `!_bids[k][a.best].bondClaimed` | 305 | `NothingToClaim()` |

State transition: marks the **winner's** `bondClaimed` (306), then transfers `a.bond` to
the maker (309). Again effects before interaction.

Event: `ForfeitClaimed(msg.sender, orderHash, bond)` (311).

`a.winnerForfeited` implies `a.best != address(0)` by construction (line 270), so the
`_bids[k][a.best]` lookup on line 304 is never a lookup against the zero address on a
reachable path.
...
`auctions(maker, orderHash)` and `bids(maker, orderHash, bidder)`
(`src/book/GlasshouseBook.sol:314-320`) return the whole structs. They exist for tests
and for a frontend; nothing in the protocol consumes them.
...
```mermaid
flowchart TD
    A["applyOutcome(query, swap, book, maxBps)"] --> B["o = IGlasshouseBook(book).outcome(query.maker, query.orderHash)<br/>STATICCALL, view"]
    B --> C{"o.status"}
    C -->|None| D["return swap unchanged<br/>plain limit order (liveness)"]
    C -->|Bidding| E["revert GlasshouseAuctionInProgress()"]
    C -->|Closed| F{"block.number <= o.exclusiveUntil"}
    F -->|no| G["return swap unchanged<br/>base price, open to everyone"]
    F -->|yes| H{"query.taker == o.winner"}
    H -->|no| I["revert GlasshouseExclusiveWindow(winner, until)"]
    H -->|yes| J{"o.clearingBps <= maxBps"}
    J -->|no| K["revert GlasshouseImprovementExceedsCap(clearingBps, maxBps)"]
    J -->|yes| L["swap.balanceIn = ceilDiv(balanceIn * (10000 + clearingBps), 10000)<br/>return swap"]
```

Line by line: `None` returns unchanged at line 65, `Bidding` reverts at line 69, the
window test is line 72, the winner test line 84, the cap test line 85, the scaling line
88, and the post-window fall-through line 94.

Two design points visible in the shape:

- `None` returning unchanged is a liveness guarantee, not laziness. A maker may ship a
  signed strategy and never open an auction; the order must still be fillable
  (`src/interfaces/IGlasshouseBook.sol:8-11`, `test/GlasshouseVM.t.sol:196-203`).
- The cap check sits *inside* the winner branch. An outsider inside the window gets
  `GlasshouseExclusiveWindow`, never `GlasshouseImprovementExceedsCap`, because the
  winner test comes first. Outside the window no scaling happens, so no cap is needed.

`maxBps` here is the maker's cap from the **signed program**, entirely separate from the
Book's own `a.maxBps`. It exists because the Book is untrusted from the program's point
of view: whatever `outcome()` claims, the price cannot move more than the maker
authorised (`src/lib/GlasshouseAuctionLib.sol:52-54`). Pinned by
`test/GlasshouseVM.t.sol:288-301`, which sets a program cap of 100 against a Book
clearing price of 250 and expects the revert.
...
```
(`src/instructions/GlasshouseAuction.sol:51-55`)

`pushHeader` writes the opcode byte and skips one byte for the length
(`InstructionBuilder.sol:21-23`). `push(address)` writes `bytes20` and advances 20
(`MemoryPtr.sol`, the `push(MemoryPtr, address)` overload). `push(uint256, 3)`
left-aligns the value into the top 3 bytes of a word and writes 3
(`MemoryPtr.sol`, `push(MemoryPtr, uint256, uint8)`; the `uint24` argument widens to
`uint256` at the call site). `patchLength` then back-patches
`end - start - 2 = 23` into byte 1.

The one-argument `build(book, maxBps)` (`GlasshouseAuction.sol:47-49`) allocates exactly
`sizeOf()` bytes and calls `resolve()`, which requires the write pointer to land exactly
on the allocation end (`MemoryPtr.sol`, `resolve`). So an off-by-one in `sizeOf` or in
either `push` width is caught at build time by `MemoryPtrStrictResolveFailed`. That is a
real check, but it only guards the writer.
...
```
(`src/routers/GlasshouseRouter.sol:26-34`)

Upstream opcode sets are an `if/else` chain in
`_runOpcode(Context memory, uint256, bytes calldata) internal virtual`, terminating in
`revert UnknownOpcode(opcode)` (`node_modules/@1inch/swap-vm/src/opcodes/AquaOpcodes.sol:27-45`;
16 dispatch arms). Extension means overriding `_runOpcode`, handling the new opcodes, and
delegating everything else to `super`. That is exactly what upstream's own
`AquaOpcodesDebug` does, so this is the sanctioned pattern rather than an invention
(`src/routers/GlasshouseRouter.sol:17-19`).

**The older `_opcodes()` array API is gone at HEAD** (`run.md` F-123). Prior hackathon
projects that appended handlers at fixed array indices were extending an API that no
longer exists, and slot numbering is now the `Opcode` enum rather than an array index.
Anyone porting that pattern will find nothing to override.

`WhitelistSequential` (0x2d) is re-added deliberately. It is not in `AquaOpcodes`, so the
deployed Aqua router cannot execute it, but the headline comparison runs the same order
under the cartel ladder and under the auction, so the shipping router has to be able to
run both (`src/routers/GlasshouseRouter.sol:22-25`). Its cost is accounted for in §10.

The router itself is thin:
...
```
(`src/routers/GlasshouseRouter.sol:47-59`)

`_dispatch` is the function pointer the VM stores in `ctx.vm.dispatch`
(`node_modules/@1inch/swap-vm/src/libs/VM.sol:26`) and calls per instruction. The linearisation
`Simulator, SwapVM, GlasshouseOpcodes` matches upstream's own router shape.

A second router exists for tests only: `GlasshouseTestRouter`, built on the **full**
`Opcodes` set plus `0x2e` (`test/helpers/GlasshouseTestRouter.sol`). It is never
deployed. It exists because `AquaOpcodes` contains neither `LimitSwap` nor
`StaticBalances` nor `DutchAuctionBalanceIn` nor `WhitelistSequential`, so the shipping
router cannot host a three-way comparison, while the test EVM does not enforce EIP-170
and the full set can (`run.md` F-135). `scripts/size-check.mjs:21` exempts it by name.
...
**67 tests pass** as of 2026-09-05 (`npm test`: `67 passing (67 solidity)`). The brief for
this document recorded 57, which was the count before `test/Comparison.t.sol` (10 tests)
landed mid-pass. The three files named in the brief account for exactly the original 57.

| File | Tests | Router | Focus |
|---|---|---|---|
| `test/GlasshouseArgs.t.sol` | 7 | none | Wire format and offsets. |
| `test/GlasshouseBook.t.sol` | 41 | none | Auction mechanism in isolation. |
| `test/GlasshouseVM.t.sol` | 9 | `GlasshouseTestRouter` | The instruction inside a real SwapVM program. |
| `test/Comparison.t.sol` | 10 | `GlasshouseTestRouter` | Identity vs clock vs bid, on one order. |

### `test/GlasshouseArgs.t.sol` (7)

| Test | Pins |
|---|---|
| `test_Layout_IsHeaderPlus23Bytes` (36) | `sizeOf() == 2 + 23`; `InstructionBuilder.sizeOf() == 2`. |
| `test_Opcode_Is0x2e_RightAfterWhitelistSequential` (42) | `opcode == 0x2e` and `WhitelistSequential == 0x2d`. Breaks on upstream renumbering. |
| `test_Build_EmitsCorrectHeader` (49) | 25 bytes total; byte 0 is `0x2e`; byte 1 is 23. |
| `test_RoundTrip_Simple` (56) | `build` then `parse` recovers both fields. |
| `testFuzz_RoundTrip` (67) | The same over the full `(address, uint24)` domain. |
| `test_RoundTrip_AllOnes_CatchesFieldBleed` (75) | Any offset slip corrupts one of two all-ones fields. |
| `test_RoundTrip_MixedExtremes` (86) | Offset slip in either direction, which all-ones alone can mask. |

### `test/GlasshouseBook.t.sol` (41)

| Behaviour | Tests | Lines |
|---|---|---|
| `open()` stores parameters immutably | `test_Open_SetsImmutableParameters` | 85 |
| Maker namespacing defeats squatting | `test_Open_IsNamespacedByMaker` | 98 |
| One auction per key, ever | `test_Open_RevertsOnReopen` | 109 |
| Zero commit or reveal window rejected | `test_Open_RevertsOnZeroCommitOrRevealWindow` | 116 |
| **Zero exclusive window rejected** (F-134 half two) | `test_Open_RevertsOnZeroExclusiveWindow` | 129 |
| `reserveBps <= maxBps < 10_000` | `test_Open_RevertsWhenReserveExceedsMax`, `test_Open_RevertsWhenMaxIsNotBelowFullBps` | 135, 141 |
| Commit guards | `test_Commit_RevertsWhenNotOpened`, `..._RevertsAfterCommitEnd`, `..._RevertsOnSecondCommitFromSameBidder` | 149, 156, 165 |
| Commit indices are monotonic | `test_Commit_AssignsMonotonicIndices` | 174 |
| Reveal phase guards | `test_Reveal_RevertsDuringCommitPhase`, `..._RevertsAfterRevealEnd` | 186, 195 |
| Reveal preimage guards | `..._RevertsWithoutCommitment`, `..._RevertsOnWrongSalt`, `..._RevertsOnDoubleReveal` | 205, 214, 240 |
| **Commitment is bound to the bidder** (no mempool replay) | `test_Reveal_CommitmentIsBoundToBidder` | 226 |
| Bid range enforced with data in the error | `test_Reveal_RevertsBelowReserveOrAboveMax` | 251 |
| Bond escrowed at reveal | `test_Reveal_EscrowsBond` | 268 |
| **Phases are disjoint in block space** | `test_Outcome_PhasesAreDisjointInBlockSpace` | 284 |
| Window end is `revealEnd + exclusiveBlocks` | `test_Outcome_ExclusiveWindowEndsAtRevealEndPlusExclusive` | 302 |
| Two bidders: winner pays second price | `test_Clearing_TwoBidders_WinnerPaysSecondPrice` | 316 |
| One bidder: pays the reserve | `test_Clearing_SingleBidder_PaysReserve` | 334 |
| Reserve is a floor | `test_Clearing_SecondBelowReserveFloorsAtReserve` | 347 |
| Zero reveals: no winner, order stays live | `test_Clearing_NoReveals_LeavesNoWinner` | 363 |
| **`reserveBps == 0` with `bps == 0` still has a winner** (F-134 half one) | `test_Clearing_ZeroReserveZeroBid_StillHasAWinner` | 378 |
| Ties resolve to the earliest commit, both reveal orders | `test_Tie_EarliestCommitWins_RevealedInCommitOrder`, `..._RevealedInReverseOrder` | 396, 412 |
| Three-way tie | `test_Tie_ThreeWay_EarliestCommitWins` | 428 |
| **Running top-2 equals a reference scan under reveal-order rotation** | `testFuzz_TopTwo_MatchesReferenceScan` | 451 |
| Only the configured router may record a fill | `test_PostTransferIn_OnlyTheConfiguredRouterMayRecord` | 496 |
| First filler sticks | `test_PostTransferIn_RecordsOnlyTheFirstFill` | 503 |
| Settle timing | `test_Settle_RevertsBeforeExclusiveWindowElapses`, `test_Settle_RevertsOnSecondSettle` | 528, 595 |
| Winner filled: no forfeit, all bonds returned | `test_Settle_WinnerFilled_NoForfeit_AndEveryoneReclaims` | 535 |
| Winner no-show: forfeits to the maker, losers unaffected | `test_Settle_WinnerNoShow_ForfeitsToMaker` | 557 |
| Filled by an outsider: winner still forfeits | `test_Settle_FilledBySomeoneElse_StillForfeits` | 583 |
| Claim guards | `test_ClaimBond_RevertsBeforeSettlement`, `..._RevertsOnDoubleClaim`, `test_ClaimForfeit_RevertsWhenWinnerDidFill` | 603, 610, 624 |
| **Escrow fully drains** | `test_Escrow_IsFullyDrainedAfterAllClaims` | 637 |

### `test/GlasshouseVM.t.sol` (9)

Program under test is `StaticBalances -> GlasshouseAuction -> LimitSwap`, the ordering §4.3
requires. Bond is 0 throughout; bond economics live in the Book tests.

| Test | Pins | Line |
|---|---|---|
| `test_NoAuction_BehavesAsAPlainLimitOrder` | `None` is liveness, not an error. | 196 |
| `test_Bidding_BlocksEveryFill` | No fill during commit *or* reveal, including after the only bid has been revealed. | 208 |
| `test_Winner_PaysSecondPrice_InsideTheWindow` | Exact improved price via the independent `_improvedOut` formula, and that it moves toward the maker. | 235 |
| `test_Outsider_IsExcludedInsideTheWindow` | `GlasshouseExclusiveWindow` with the exact winner and `until`. | 249 |
| `test_AfterTheWindow_OrderIsOpenToEveryoneAtBasePrice` | Exclusivity expires for the outsider **and** the obligation expires for the winner. | 267 |
| `test_ImprovementCapIsEnforcedAgainstTheBook` | The signed cap overrides the Book. | 288 |
| **`test_QuoteEqualsSwap`** | The invariant of §7, including realised balance deltas. | 308 |
| `test_QuoteIsStableAcrossBlocksInsideTheWindow` | Frozen storage means no drift across the window. | 334 |
| `test_Swap_RecordsTheFillThroughTheMakerHook` | Hook fires on `swap()`, never on `quote()`. | 354 |

### `test/Comparison.t.sol` (10)

Landed after this document's brief; summarised rather than mapped in full. Runs one order
under all three allocation rules on `GlasshouseTestRouter`: `test_Identity_*` (3) for
`WhitelistSequential`, `test_Clock_*` (3) for `DutchAuctionBalanceIn`,
`test_Bid_*` (2) for Glasshouse, plus `test_Comparison_AllThreeGatesOnTheSameOrder` and
`testFuzz_Comparison_ClockNeverBeatsBaseForTheMaker`.

### Not covered

- No test exercises a maker who opens an auction with a **wrong or zero `router`**, or who
  signs an order **without** MakerTraits bit 251. Both make `filledBy` permanently zero and
  both cause an honest winner to forfeit. See G-3.
- No test exercises a bidder who commits and deliberately withholds a reveal after
  observing other reveals. See G-4.
- No test exercises `bond > 0` through a full `quote`/`swap` at the router level; the VM
  tests set `bond = 0` and the Book tests call `postTransferIn` directly under a prank.
- No test asserts what happens when `GlasshouseAuction` is placed **before**
  `StaticBalances` or **after** `LimitSwap`. Both misorderings fail silently (§4.3), so
  the constraint is documented but not enforced by a test.
